...
# --- NEW: raycasts ahead and steers the direction away from walls ---
func _avoid_walls(from_pos: Vector2, desired_dir: Vector2) -> Vector2:
	...
	if result:
		# Try steering left and right of the desired direction, pick whichever is clear
		...
		if not right_hit:
			...
			# Both sides blocked too — bounce off the wall normal (turn around)
...
func _wander(delta: float) -> void:
	...
	if direction.length() > 4.0:
		...
		# If we had to steer significantly, retarget the wander point in that direction
		# so it doesn't immediately try to walk back into the wall next frame
